...
| SHOW_APPLICATION                                         | Used by embedded applications to register themselves with Canvas. Returned in response to `APPLICATION__ON_GET` events. Check out [Embedded Applications](/sdk/handlers-applications/#embedded-applications). |
...
| UPSERT_COMMAND_METADATA                                  | Can be used to add or update metadata on a command. Check out [CommandMetadata Effect](/sdk/effect-command-metadata/).                                                                                     |
