...
- If more than one handler responds with a `ProviderMenuConfiguration` effect, the last effect returned wins — Canvas applies its allow-list and overwrites the earlier responses.
- If an allow-list that reaches the backend contains an item Canvas does not recognize, the server discards the entire list and renders all default native items instead of a partial menu. This is fail-safe, and distinct from the construction-time `ValidationError` above, which rejects non-enum values before they leave the plugin.
- If the plugin fails while resolving the menu configuration, Canvas renders all default native items rather than hiding anything, so a broken plugin never leaves a provider with an empty or broken menu.
