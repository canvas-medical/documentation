...
Temporarily freezes a note to prevent modifications by other users. The note will display a banner indicating it is frozen and will be read-only for users other than the lock owner. The freeze is automatically released after the specified duration.

### FreezeNoteEffect

Freezes a note for a specified duration. The user who initiated the freeze (the lock owner) can continue editing while others see a read-only view.
...
| `duration` | `int`           | Freeze duration in seconds                               | No       | 300     |
| `user_id`  | `str` or `None` | Staff key of user who initiated freeze (can still edit)  | No       | None    |
| `blur`     | `bool`          | Whether to blur note content for other users             | No       | False   |

#### Implementation Details

- The freeze state is stored in the note's `related_data` field under a `freezeState` key
- The lock owner (specified by `user_id`) can continue editing while the note is frozen
- Other users will see a "Note Frozen" banner and cannot make changes
- If `blur` is enabled, the note content is visually blurred for other users
- A Celery task automatically unfreezes the note after the duration expires
- Re-freezing by the same user extends the timer without re-broadcasting to other clients
...
Manually unfreezes a note before the automatic duration expires. This removes the freeze state entirely.
...
- Removes the `freezeState` from the note's `related_data`
- Revokes any pending auto-unfreeze Celery task
...
class NoteHandler(BaseHandler):
    ...
    def compute(self):
        return [
            UnfreezeNoteEffect(note_id=self.target).apply()
        ]
```
