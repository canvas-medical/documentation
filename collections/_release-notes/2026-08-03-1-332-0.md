...
- Plugins can now show a notification badge — a count (capped at "99+") with a bolded label — on a `provider_menu_item` scoped application's item in the provider menu (the navigation sidebar, also called the hamburger menu), using the new [Application Notification Badge](/sdk/effect-application-notification-badge/) effect or the [`compute_notification_badge`](/sdk/handlers-applications/#notification-badges) hook on the Application handler.
