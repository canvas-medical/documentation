...
- Inbound faxes will try to find an existing service provider contact before creating to prevent duplicate entries.
...
- Adds a new pdf generator utility that enables plugins to generate PDFs from a hosted HTML page via or  from raw HTML content.
- Add Python 3.13 support to the SDK.
- Creates new claim effect for updating provider information on a claim.
